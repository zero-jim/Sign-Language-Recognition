function result = batchPreProcess(inDir, outDir)
%batchPreProcess Batch process images using PreProcess
% RESULT = batchPreProcess(INDIR, OUTDIR) processes each file in INDIR
% using the function PreProcess.
%
% The following fields from the output of PreProcess are written with their
% corresponding file format to the output directory OUTDIR:
%    BW saved as tiff format
%    E saved as tiff format
%    im saved as tiff format
%
...
if(nargin<1)
    inDir = 'E:\Thesis\Final Thesis\Works\Code\Practice\Bangla_Dataset';
end
...
% Process each image using PreProcess
